<?php /* Smarty version Smarty-3.1.19, created on 2016-02-18 15:52:03
         compiled from "./templates/footer_modifica_2.tpl" */ ?>
<?php /*%%SmartyHeaderCode:10266040845652cfcde0f744-11798901%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '9e7ac2a3692fc3737775601c67fc86073faff1a6' => 
    array (
      0 => './templates/footer_modifica_2.tpl',
      1 => 1455795610,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '10266040845652cfcde0f744-11798901',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.19',
  'unifunc' => 'content_5652cfcde43e82_42112150',
  'variables' => 
  array (
    'esito' => 0,
    'agenda' => 0,
...
<?php if ($_valid && !is_callable('content_5652cfcde43e82_42112150')) {function content_5652cfcde43e82_42112150($_smarty_tpl) {?><?php if (!is_callable('smarty_modifier_date_format')) include '/var/www/vhosts/beecloud.it/easysmile.beecloud.it/panel/libs/plugins/modifier.date_format.php';
?></div>
</div>
</div>
</div>
...
<script src="assets/fullcalendar/it.js"></script>
    ...
    $('img[usemap]').maphilight({ fillColor: 'a94442'});
$("map area").on('click', function () {
    var num = "#"+$(this).attr('id');
    var numa = "#"+$(this).attr('id')+"a";
    var numb = "#"+$(this).attr('id')+"b";
$(numa).removeClass('hidden');
    var data = $(num).mouseout().data('maphilight') || {};
            if(data.alwaysOn !==true){
                $(numa).removeClass('hidden');
                $(numb).val('true');
            }
            else{
                $(numa).addClass('hidden');
                $(numb).val('false');
            }
            data.alwaysOn = !data.alwaysOn;
            $(num).data('maphilight', data).trigger('alwaysOn.maphilight');
});
...
');<?php }?>
    ...
    $('#paciente.select2-offscreen').empty();
            $('#paciente').html('<option value="All" disabled="disabled">Seleziona paziente...</option>');
            $('#paciente').select2('val', 'All');
            var selectedCountry = $("#studio option:selected").val();
            var options = $("#paciente");
            $.ajax({
            type: "POST",
                    url: "province.php",
                    data: { paciente : selectedCountry }
            }).done(function(data){
