...
{if $smarty.session.utente['idRuoloUtente'] != $smarty.const.AMMINISTRATORE} 
    ...
    width: '100%',
        ...
        {foreach $agenda as $key=>$value}
                            {
                                title: "{if $value['fkStatoAppuntamento'] == '1'} - {$value['dataFine']|date_format:"H:i"}{else} {if $smarty.session.utente['idRuoloUtente'] == $smarty.const.PAZIENTE}{$value['tipoAppuntamento']} {else}{$value['nome']} {$value['cognome']}{/if}{/if}",
